...
exports.getAllMovies = async (req, res) => {
  ...
  const movies = await Movie.aggregate(userMoviesPipeline(type));
  ...
  const AllMovies = await Promise.all(movies.map(mapMovies));
...
exports.searchGenre = async (req, res) => {
  ...
  if (!genre.trim()) return sendError(res, "Invalid request!");
...
exports.searchMovietitle = async (req, res) => {
  const { title} = req.query;
